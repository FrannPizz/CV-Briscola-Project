...
/*
the briscola is the same for the whole game and stays still under the deck in rounds 1..17,
a played card appears only in its round: the briscola is the card seen in more rounds IN THE SAME PLACE.
Only the rounds where the label is close to its usual position (median) are counted: a card back of the piles
read by mistake as a card is seen in different places, so it does not collect many rounds.
...
int findBriscola(const std::vector<CardDetection>& detections, const MatchParams& p)
{
    ...
    //for every label: in how many rounds (1..17) it is seen close to its usual position
    std::vector<int> rounds(41, 0); //41 because the labels are 1..40, index 0 is unused
    for (int label = 1; label <= 40; ++label) {

        //usual position of the label (median of its boxes in rounds 1..17)
        cv::Point2f position = findBriscolaPosition(detections, label, nRounds);

        //seen[round] = true if the label is recognized close to that position in that round
        std::vector<bool> seen(nRounds + 1, false);
        for (int i = 0; i < detections.size(); ++i) {
            const CardDetection& d = detections[i];
            if (d.label == label && briscolaOnTable(d.round, nRounds) && cv::norm(d.center - position) < p.briscolaDistance)
                seen[d.round] = true;
        }
...
/*
position of the briscola under the deck, for reject false positives of the briscola label
...
//true if the box is the briscola under the deck (not a played card): rounds 1..17 
...
//identifies the second card played in the round: the nearest cluster to the first one, that appears after it and is seen in at least 2 frames
...
/*
find themiddle of the table between North and South, used when a round has only one card detetcted to determine if it is North or South.
